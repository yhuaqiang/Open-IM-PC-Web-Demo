...
import LoginForm, { FormField, InfoField } from "./components/LoginForm2";
...
import api from '../../extra/dalao-api';
...
const Login = () => {
  ...
  const login = (data: FormField) => {
    ...
    const config = {
      ...
      passOpenid: true,
      // dev
      APPID: 'wx46d5f0a7fe6b2052',
      envId: 'mydev-6gty8rjp3f70af8c',
    };
    /* prod
    Object.assign(config, {
      APPID: 'wxecfd44ca5649a44a',
      envId: 'ama000-4goqjxsz95c03704',
    })
    //*/
    ...
    api.docall('admin.login', { username: data.phoneNo, password: md5(data.password as string) }, false)
      .then((res) => {
        if (res.ok) {
  ...
  const imLogin = async (userID: string, token: string) => {
    ...
    im.login(config)
      .then((res) => {
        console.log('im.login', config, res);
